...
_Listiterator(Node* node):_node(node)
  {}
//*it
Ref& operator *(){//出了作用域对象还在,T是结点里面的数据
return _node->_data;
}
Ptr* operator->(){
...
_Listiterator<T,T&,T*>& operator++(){//迭代器++还是迭代器,返回++以后的值
...
//it1!=it2
bool operator !=(const _Listiterator<T,T&,T*>& it){//!=可以让迭代器跑起来,迭代器是否相等是迭代器对象比较
return _node!=it.node;
}

_Listiterator<T,T&,T*> &operator++(int){//必须传值返回
  _Listiterator<T,T&,T*> tmp(*this);
_node=_node->_next;
return tmp;
}
_Listiterator<T,T&,T*>& operator--(){
_node=_node->_preve;
return *this;
}
_Listiterator<T,T&,T*>& operator--(int){
_Listiterator<T,T&,T*> tmp(*this);
_node=_node->_preve;
return tmp;
}
